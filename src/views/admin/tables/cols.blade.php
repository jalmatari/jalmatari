        ...
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        حقول جدول "{{$table->title==''?$table->name:$table->title}}"

                    </h3>
                </div>
                {!!Funs::Form('open',[['route' =>['admin.tables.cols',$table->id],'role'=>"form"]])!!}
                    ...
                    <div class="multi-fields">
                        @foreach($cols as $col)
                            <div class="multi-field {{$col->COLUMN_NAME.'_row '.($loop->first?'not-sortable':'')}} col-md-12"
                                ...
                                <div class="form-group col-md-2">
                                    {!! Funs::Form('hidden',['ID[]',Funs::IsIn($col,'ID',null)]) !!}
                                    {!! Funs::Form('text',['COLUMN_NAME[]',$col->COLUMN_NAME,['class'=>'form-control','dir'=>'ltr']]) !!}
                                    {!! Funs::Form('hidden',['OLD_COLUMN_NAME[]',$col->COLUMN_NAME]) !!}
                                </div>
                                <div class="form-group col-md-2">
                                    {!! Funs::Form('text',['TITLE[]',$col->TITLE,['class'=>'form-control']]) !!}
                                </div>
                                <div class="form-group col-md-1">
                                    {!! Funs::Form('select',['TYPE[]',Funs::$ColsTypes,$col->TYPE,['class'=>'un-advanced-select form-control']]) !!}
                                </div>
                                <div class="form-group col-md-2">
                                    <?php
                                    $source = $col->SOURCE;
                                    ...
                                    echo Funs::Form('select', [ 'SOURCE[]', [ null, 'function' => 'دالة' ], $source, [ 'class' => 'un-advanced-select form-control' ] ]);

                                    ?>
                                    <div id="fun-pars" {!! $isFunction?'':'style="display:none"' !!}>
                                        {!! Funs::Form('label',[ 'source_fun_name_'.$col->COLUMN_NAME,'اسم الدالة:']) !!}
                                        {!! Funs::Form('select',['source_fun_name_'.$col->COLUMN_NAME,$sourceFuns,Funs::IsIn($col->SOURCE,'function',null),['class'=>'un-advanced-select form-control']]) !!}
                                    </div>
                                </div>
                                <div class="form-group col-md-1">
                                    {!! Funs::Form('checkbox',['SHOW_IN_LIST_'.$col->COLUMN_NAME,null,$col->SHOW_IN_LIST]) !!}
                                </div>
                                <div class="form-group col-md-3 attrs">
                                    @if ($col->ATTR != '')
                                        <?php $attrs = json_decode($col->ATTR); ?>
                                        @if (count($attrs) >= 1)
                                            @foreach ($attrs as $key => $attr)

                                                <div class="attr">
                                                    <div class="form-group col-xs-4">
                                                        {!! Funs::Form('text',['attr_key_'.$col->COLUMN_NAME.'[]',$key,['class'=>'form-control','dir'=>'ltr','placeholder'=>'الخاصية']]) !!}
                                                    </div>
                                                    <div class="form-group col-xs-7">
                                                        {!! Funs::Form('text',['attr_val_'.$col->COLUMN_NAME.'[]',$attr,['class'=>'form-control','placeholder'=>'القيمة']]) !!}
                                                    </div>
    ...
    <script>
        ...
        function addAttr(colName, key, val) {
            ...
            attrRow = '<div class="attr">'
                + '<div class="form-group col-xs-4">'
                + '<input class="form-control" dir="ltr" name="attr_key_' + colName + '[]" type="text" value="' + key + '" placeholder="الخاصية">'
                + '</div>'
                + '<div class="form-group col-xs-7">'
                + '<input class="form-control" name="attr_val_' + colName + '[]" type="text" value="' + val + '" placeholder="القيمة">'
                + '</div>'
                + '<div class="form-group col-xs-1">'
                + '<a href="#" class="bg-red remove-attr pull-left">'
                + '<i class="fa fa-remove"></i>'
